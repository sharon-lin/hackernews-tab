...
function HubTab() {
    ...
    var trendingRequest = false,              // To make sure that there are no parallel requests
        ...
        reposApiUrl = 'https://hn.algolia.com/api/v1/search_by_date', // URL for the repos
     ...
     * @returns {string}
     */
    function generateReposHtml(stories) {
        var html = '';

        $(stories).each(function (index, story) {
          if (story.title && story.url){
            // Make the name and description XSS safe
            var repFullName = $('<div>').text(story.title).html();
            if (story.url.indexOf("www") >=0 && story.url.indexOf("www")<=12){
              var firstnum = story.url.indexOf("www")+4;
            }
            else if (story.url.indexOf("https://www") >=0){
              var firstnum = 14;
            }
            else if (story.url.indexOf("https") >=0){
                var firstnum = 8;
            }
            else{
              var firstnum = story.url.substring(story.url.indexOf("//")+2);
            }
            if (story.url.substring(firstnum).indexOf("/") > 0){
              var secondnum = story.url.substring(firstnum).indexOf("/")+firstnum;
            }
            else{
              var secondnum = story.url.length;
            }
            var repFullDesc = $('<div>').text(story.url.substring(firstnum,secondnum)).html();
    ...
    var fetchTrendingRepos = function () {
        ...
        trendingRequest = $.ajax({
            ...
            success: function (data) {
                var finalHtml = generateReposHtml(data.hits);
